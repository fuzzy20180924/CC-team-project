...
public class MainVerticle extends AbstractVerticle {
    ...
    private static String userQuery = "SELECT * "
      + " FROM " + "user"
      + " WHERE " + "id" + " in (";
    private static String q3Query = "SELECT *" + " FROM q3"
  ...
  // in browser access http://localhost:8080/q2?user_id=1608836484&phrase=Counter%20Spy&n=5
    public void start() throws Exception {
        ...
        JsonObject config = new JsonObject()
          .put("port", 3306)
          .put("host", MYSQL_HOST)
          .put("username", MYSQL_NAME)
          .put("password", MYSQL_PWD)
          .put("database", DB_NAME)
          .put("sslMode", "disable")
          .put("charset", "UTF-8")
          .put("maxPoolSize", 1500);
        SQLClient jdbc = MySQLClient.createShared(vertx, config);
	    ...
	    try {
            jdbc.getConnection(aconn->{
		      ...
		      try {
                      ...
                      query1.compose(v ->{
                          String output = TEAMID + "," + TEAM_AWS_ACCOUNT_ID + "\n";
                          Q3MySQL q3 = new Q3MySQL();
                          String r = q3.getResponse(v.getRows(), n1,n2);
                          output += r;
                          response.putHeader("content-type", "text/html")
                          .end(output.substring(0, output.length()));
                          conn.close();
                          fi.complete();},fi);
		      } catch (Exception e) {
			  conn.close();
		          HttpServerResponse rresponse3 = routingContext.response();
                     	  rresponse3.putHeader("content-type", "text/html")
                         	.end(TEAMID+","+TEAM_AWS_ACCOUNT_ID);	  
		      }	      
                } else {
		     ...
		     HttpServerResponse rresponse5 = routingContext.response();
                          rresponse5.putHeader("content-type", "text/html")
                                .end(TEAMID+","+TEAM_AWS_ACCOUNT_ID);
            }
        });
	    ...
	    try {
		  ...
		  if (v == null) {
		     HttpServerResponse rresponse = routingContext.response();
		     rresponse.putHeader("content-type", "text/html")
			 .end(TEAMID+","+TEAM_AWS_ACCOUNT_ID);    
		  } else {	  
		  ...
		  conn.close();
	              ...
	              conn.close();
		      HttpServerResponse rresponse2 = routingContext.response();
                     rresponse2.putHeader("content-type", "text/html")
                         .end(TEAMID+","+TEAM_AWS_ACCOUNT_ID);
		  }  
              } else {
                routingContext.fail(aconn.cause());
		    ...
		    HttpServerResponse rresponse4 = routingContext.response();
                     rresponse4.putHeader("content-type", "text/html")
                         .end(TEAMID+","+TEAM_AWS_ACCOUNT_ID);
            }
